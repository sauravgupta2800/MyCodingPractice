...
int main()
{
    ...
    int b[bucket],loc_of_b;
    memset(b,0,sizeof(b));
    int avg_size=ceil(float(n)/bucket);
    int last_size=n-((bucket-1)*avg_size);
    //cout<<last_size<<endl;
    //cout<<n<<"  "<<bucket<<" "<<avg_size<<endl;
    int loc=-1;
    for(int i=0;i<n;i++)
    {
        loc_of_b=(arr[i]-1)/avg_size;
        {
            b[loc_of_b]=b[loc_of_b]+1;
            if(b[loc_of_b]>avg_size || b[bucket-1]>=last_size)
            {
                loc=loc_of_b;//when overflow it must entered
                break;
            }
        }
    }
    ...
    if(loc==-1)
    {
        cout<<"no repetetion"<<endl;
    }
    //else
    {
        int f_index=loc*avg_size+1;
        int l_index;
        if(loc==(bucket-1))//for last bucket
        {
            l_index=f_index+last_size-1;
        }
        else
        {
            l_index=f_index+avg_size-1;
        }
        cout<<"F:"<<f_index<<" L:"<<l_index<<endl;
        /*
        int diff=l_index-f_index+1;
        int hash[diff];
        memset(hash,diff,sizeof(hash));
        */
        int count=0;
        int val;
        for(val=f_index;val<=l_index;val++)
        {
            int count=0;
        ...
        cout<<"value repeated is: "<<val<<endl;
    }
}
